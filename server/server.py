...
def _load_public_tools() -> list[dict]:
    payload = json.loads(Path("generated_mcp/automotive-operations-tools.json").read_text(encoding="utf-8"))
    public_tools = []
    for tool in payload["tools"]:
        item = dict(tool)
        item.pop("action_kind", None)
        item["securitySchemes"] = [{"type": "noauth"}]
        item["_meta"] = {
            "securitySchemes": [{"type": "noauth"}],
            "ui": {"resourceUri": "ui://widget/universal-industry-content-v1.html"},
            "openai/outputTemplate": "ui://widget/universal-industry-content-v1.html",
        }
        public_tools.append(item)
    return public_tools


TOOLS = _load_public_tools()
